      ...
      <div class="infoUser">
        <div class="md-layout-item md-size-80 md-medium-size-100">
          <tabs :tab-name="[
            ...
            <template slot="tab-pane-1">
              <p>
                <md-icon>person</md-icon> {{ user.name }}
              </p>
            </template>
            <template slot="tab-pane-2">
              <p>
                <md-icon>link</md-icon> {{ user.id }}
              </p>
            </template>
            <template slot="tab-pane-3">
              <p>
                <md-icon>alternate_email</md-icon> {{ user.email }}
              </p>
            </template>
            ...
            <template slot="tab-pane-5">
              <p>
                <md-icon>call</md-icon>{{ user.telephone }}
              </p>
            </template>
            <template slot="tab-pane-6">
              <p>
                <md-icon>info</md-icon> {{ user.info }}
              </p>
            </template>
          </tabs>
        </div>
        <div class="md-layout-item md-size-20 md-medium-size-100">
          <div class="buttonsPlatform">
            ...
            > -->
            <md-button class="md-warning" @click="displayEditUser()">Edit User</md-button>
            <md-button class="md-warning" @click="deleteUser()">Delete User</md-button>
          </div>
        </div>
      </div>
      ...
      <div class="md-layout-item md-size-100">
        <md-card>
          <md-card-header class="md-card-header-icon md-card-header-green">
            ...
            <div class="button">
              <md-button @click.native="showTable((itemTable = 'Platforms'))">Platforms</md-button>
              <md-button @click.native="showTable((itemTable = 'Logs'))">Logs</md-button>
            </div>
          </md-card-header>
